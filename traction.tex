...
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
    ...
    \node [block, right of = addspeedsetpoint, xshift = 1cm](strip){$G(p)?$};
...
To solve this, we refine our gray-box approximation by introducing an integrator in the system, computing its response to $\tilde{\Delta_\omega}(t)$ and trying to determine the rest of the dynamics based on this new input and the observed response, as shown in figure \ref{fig:tractionGrayBox}, where $G(p) = \frac{1}{p} G'(p)$.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
    % We start by placing the blocks
    \node [input](input){};
    \node [square, right of = input, xshift = 1cm](integrator){$\frac{1}{p}$};
    \node [block, right of = integrator, xshift = 2.5cm](strip){$G'(p)?$};
...
Fitting a simple first order transfer function to $G'(p)$ is not easy because the dynamics between $\frac{\tilde{\Delta_\omega}(p)}{p}$ and $F_R(p)$ is very fast, as shown in figure \ref{fig:tractionPulseStep}. This leads to very large poles which are not well approximated and difficult to simulate, as previously stated.
\begin{figure}[htbp]
\centering
...
To solve this, we tried to fit $G'(p)$ with a transfer function of the form $K\frac{s-z_0}{s-p_0}$, because the introduction of a zero speeds up a step response, which would bring $p_0$ reasonably closer to the origin. The result is shown in figure \ref{fig:tracFit}, where we see that the following transfer function seems to fit the experience very well.
\[G'(p) = 13.096\frac{s+0.9221}{s(s+4.063)}\]
\begin{figure}[htbp]
\centering
